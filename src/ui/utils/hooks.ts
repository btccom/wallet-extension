...
export const useNotice = () => {
  ...
  const getNotice = wallet.getNotice;
  const resolveNotice = async (data?: any, stay = false, forceReject = false) => {
    const notice = await getNotice();
    if (notice) {
      wallet.resolveNotice(data, forceReject);
    }
    if (stay) {
      return;
    }
  ...
  const rejectNotice = async (err?, stay = false, isInternal = false) => {
    const notice = await getNotice();
    if (notice) {
      await wallet.rejectNotice(err, stay, isInternal);
    }
    if (!stay) {
      navigate('/');
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener('beforeunload', rejectNotice);

    return () => window.removeEventListener('beforeunload', rejectNotice);
  }, []);

  return [getNotice, resolveNotice, rejectNotice] as const;
};
