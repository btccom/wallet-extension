...
import { AddressType, MemStoreState } from '@/common/types';
import { HdKeyring } from '@/scripts/background/libs/hdKeyring';
import commonManager from '@/scripts/background/libs/manager/commonManager';
import { SimpleKeyring } from '@/scripts/background/libs/simpleKeyring';
import { ObservableStore } from '@metamask/obs-store';

export interface Keyring {
  signMessage(publicKey: string, message: string, addressType: AddressType): Promise<any>;
  signMessageByHdPath(hdPath: string, index: number, message: string): unknown;
  type: string;
...
class DisplayKeyring {
  ...
  type = '';
  hdPath = '';
  walletKeyManager: any = {};

  constructor(keyring: Keyring, walletKeyManager) {
  ...
  unlock = async (): Promise<void> => {
    const keyring = await this.walletKeyManager.getKeyringForAccount(this.accounts[0], this.type);
...
class EmptyKeyring implements Keyring {
  type = KEYCHAIN_TYPE.Empty;
  constructor() {
    // todo
  }
  signMessage(publicKey: string, message: string): Promise<any> {
...
class WalletService extends EventEmitter {
  ...
  _restoreKeyring = async (serialized: any): Promise<{ keyring: Keyring; addressType: AddressType }> => {
    ...
    if (type === KEYCHAIN_TYPE.Empty) {
      const keyring = new EmptyKeyring();
      return { keyring, addressType: addressType === undefined ? commonManager.getAddressType() : addressType };
    }
    const Keyring = this.getKeyringClassForType(type);
  ...
  getAddressTypeForAccount = async (
    address: string,
    type?: string,
  ): Promise<AddressType> => {
    log.debug(`KeyringController - getKeyringForAccount: ${address}`);
    ...
    for (let i = 0; i < keyrings.length; i++) {
      const keyring = keyrings[i];
      const addressType = this.addressTypes[i];
      const accounts = await keyring.getAccounts();
      if (accounts.includes(address)) {
        return addressType;
      }
  ...
  displayForKeyring = async (keyring: Keyring, addressType: AddressType, index: number): Promise<DisplayedKeryring> => {
    ...
    return {
      ...
      keyring: new DisplayKeyring(keyring, this),
      addressType,
      index
    };
  };
  ...
  signMessage = async (address: string, message: string) => {
    const keyring = await this.getKeyringForAccount(address);
    const addressType = await this.getAddressTypeForAccount(address);
    const data = await keyring.signMessage(address, message, addressType);
    return data;
  };
}
